...
from ocr_engines.detect import detect_document_type, detect_document_types
...
def ocr_endpoint():
    ...
    if data and data.get("file_b64"):
        ...
        try:
            ...
            save_name = None
            # 1) if provided filename with extension, use it
            if provided_name and Path(provided_name).suffix:
                ...
                if raw[:4] == b"%PDF":
                    ...
                    # 3) try to open as image via PIL — if success, save as PNG
                    try:
                        ...
                        # fallback: unknown binary -> save raw bytes with .bin so iterate_pages_from_path will error or be explicit
    ...
    try:
        for page_index, bgr_img, _temp in iterate_pages_from_path(file_path):
            ...
            try:
                ...
                page_clean = f"[error on page {page_index+1}: {e_page}]"

            try:
                detected = detect_document_types(page_clean) if 'detect_document_types' in globals() else detect_document_type(page_clean)
                ...
                try:
                    single = detect_document_type(page_clean)
                    detected = [single] if single else ["unknown"]
                except Exception:
                    detected = ["unknown"]
    ...
    # Combine text preview for compatibility
    combined_pages = []
    for p in page_results:
        combined_pages.append(f"--- PAGE {p['page']} ---\n{p['text']}\n")
    combined_text = "\n".join(combined_pages)
    ...
    # Aggregate overall document types across pages
    overall_types = []
    for p in page_results:
        for dt in p.get("document_types", []):
            if dt not in overall_types:
                overall_types.append(dt)
    ...
    response = {
        ...
        "pages": page_results,
        "overall_document_types": overall_types
    }
